<?php
 ...
 function goster_kategori($id = ""){
    $sql = @mysql_query("SELECT * FROM kategori");
 	...
 	$sqlb = @mysql_query("SELECT id,ad_soyad FROM aday WHERE kategori='{$id}' AND durum='1' ORDER by 'ad_soyad' ASC");
        ...
        <tr>
          <td valign="top" width="33%">
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                <tr>
                  <td bgcolor="#466176" height="35"><font color="white" size="2px"><strong>Aday göstermek istediðiniz penguenler mi var?</strong></font></td>
                </tr>
                <tr>
                  <td> </td>
                </tr>
                <tr>
                  <td class="menu"><font>Eðer aday göstermek istediðiniz penguenler varsa, <strong><a href="aday_ekle.php?id=<?=$bga['id']?>">Aday Ekleme Formu</a></strong>'nu doldurarak aday listesine ekleyebilirsiniz.</font></td>
                </tr>
              </tbody>
          </table></td>
        </tr>
 ...
 function goster_aday_liste(){
  ...
  while($bgs = @mysql_fetch_assoc($sql)){
   $sqll = @mysql_query("SELECT * FROM aday WHERE kategori='{$bgs['id']}' AND durum='1' ORDER by 'ad_soyad' ASC");
   ?>
   <table align="center" border="0" cellpadding="4" cellspacing="1" width="96%">
      <tbody>
        <tr>
          <td valign="top" width="33%">
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                <tr>
                  <td bgcolor="#466176" height="35"><font color="white" size="2px"><strong><?=$bgs['adi']?> Adaylarý</strong></font></td>
                </tr>
                <tr>
                  <td> </td>
                </tr>
 ...
 function goster_aday($id = ""){
  $sql = @mysql_query("SELECT * FROM aday WHERE id='{$id}' AND durum='1'");
  $sqlw = @mysql_query("SELECT * FROM aday_yorum WHERE aday_id='{$id}' AND aktif='1'");
        ...
        <tr>
          <td valign="top" width="33%">
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                <tr>
                  <td bgcolor="#466176" height="35" class="menu"><font color="white" size="2px"><strong><?=$bg['ad_soyad']?> (<?=kat_adi($bg['kategori'])?> Adayý)</strong></font></td>
                </tr>
                <tr>
                  <td> </td>
                </tr>
 ...
 function aday_ekle($aday,$kategori,$oneren,$oneren_eposta,$oneri_metni){
  if(isset($aday) AND isset($kategori) AND isset($oneren) AND isset($oneren_eposta) AND isset($oneri_metni)){
   $sql = @mysql_query("INSERT INTO `aday` (`id`,`ad_soyad`,`kategori`,`oneren_ad`,`oneren_eposta`,`oneri_metni`,`durum`) VALUES ('', '{$aday}', '{$kategori}', '{$oneren}', '{$oneren_eposta}', '{$oneri_metni}', '0')");
  }
  if($sql){$ok = "pl";}
  elseif(!$sql){$ok = "pll";}
  else{$ok = "pll";}
  ...
  if($sql){
   ...
   $m->From(SITEMAIL);
   $m->To (ADMINMAIL);
   $m->Subject ("Yýlýn Penguenleri Sitesi");
   $m->Body ("Merhaba\n\nLKD Yýlýn Penguenleri Ödülleri Sitesi'ne [1] yeni bir aday eklendi, ilgilenebilirsen [2] sevinirim.\n\n[1] http://odul.linux.org.tr/2005/\n[2] http://odul.linux.org.tr/2005/yonetim/\n\nÝyi çalýþmalar");
   $m->Priority (1);
   $m->Send ();
  }
 ...
 function penguen_adi($id){
  $sql = @mysql_query("SELECT id,ad_soyad FROM aday WHERE id='{$id}'");
 ...
 function yorum_ekle($penguen,$yorum_adi,$yorum_eposta,$yorum){
  if(isset($penguen) AND isset($yorum_adi) AND isset($yorum_eposta) AND isset($yorum)){
   $sql = @mysql_query("INSERT INTO `aday_yorum` (`id`,`aday_id`,`ekleyen_ad`,`ekleyen_eposta`,`yorum`,`tarih`,`aktif`) VALUES ('', '{$penguen}', '{$yorum_adi}', '{$yorum_eposta}', '{$yorum}', '', '0')");
  }
  if($sql){$ok = "pl";}
  elseif(!$sql){$ok = "pll";}
  else{$ok = "pll";}
  ...
  if($sql){
   ...
   $m->To (ADMINMAIL);
   $m->Subject ("Yýlýn Penguenleri Sitesi");
   $m->Body ("Merhaba\n\nLKD Yýlýn Penguenleri Ödülleri Sitesi'nde [1] yeni bir yorum eklendi, ilgilenebilirsen [2] sevinirim.\n\n[1] http://odul.linux.org.tr/2005/\n[2] http://odul.linux.org.tr/2005/yonetim/\n\nÝyi çalýþmalar");
   $m->Priority (1);
   $m->Send ();
  }
        ...
        <tr>
          <td valign="top" width="33%">
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                <tr>
                  <td bgcolor="#466176" height="35"><font color="white" size="2px"><strong>Onaylanmayý bekleyen yorumlar</strong></font></td>
                </tr>
                <tr>
                  <td> </td>
                </tr>
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM aday_yorum WHERE aktif='0'");
            ...
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                ...
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM aday_yorum WHERE aktif='1' ORDER by 'id' ASC");
 ...
 function onayla_yorum($id,$islem){
  if($islem == 1){
   $sql = @mysql_query("UPDATE aday_yorum SET aktif='1' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=yorum"><?
  }
  elseif($islem == 0){
   $sql = @mysql_query("DELETE FROM aday_yorum WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=yorum"><?
  }
  elseif($islem == 2){
   $sql = @mysql_query("UPDATE aday_yorum SET aktif='0' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=yorum"><?
  }
  else{
        ...
        <tr>
          <td valign="top" width="33%">
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                <tr>
                  <td bgcolor="#466176" height="35"><font color="white" size="2px"><strong>Onaylanmayý bekleyen adaylar</strong></font></td>
                </tr>
                <tr>
                  <td> </td>
                </tr>
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM aday WHERE durum='0'");
  
  while($bgl = @mysql_fetch_assoc($sqlw)){
  ?>
                  <strong><?=$bgl['ad_soyad']?> adlý penguen "<a href="mailto:<?php echo str_replace("@"," [at] ",$bgl['oneren_eposta']); ?>"><?=$bgl['oneren_ad']?></a>" tarafýndan "<?php echo kat_adi($bgl['kategori']); ?>" kategorisinde öneriliyor:</strong><br /> <?php echo unhtmls($bgl['oneri_metni']); ?>
        ...
        <tr>
          <td valign="top" width="33%">
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                <tr>
                  <td bgcolor="#466176" height="35"><font color="white" size="2px"><strong>Onaylanmýþ (gösterimdeki) adaylar</strong></font></td>
                </tr>
                <tr>
                  <td> </td>
                </tr>
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM aday WHERE durum='1' ORDER by 'ad_soyad' ASC");
 ...
 function onayla_aday($id,$islem){
  if($islem == 1){
   $sql = @mysql_query("UPDATE aday SET durum='1' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=aday"><?
  }
  elseif($islem == 0){
   $sql = @mysql_query("DELETE FROM aday WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=aday"><?
  }
  elseif($islem == 2){
   $sql = @mysql_query("UPDATE aday SET durum='0' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=aday"><?
  }
  else{
   ?><meta http-equiv="refresh" content="0;URL=index.php"><?
 ...
 function penguen_bilgi($id){
  $sql = @mysql_query("SELECT * FROM aday WHERE id='{$id}'");
  $assoc = @mysql_fetch_assoc($sql);
  return $assoc;
 }
 
 function yorum_bilgi($id){
  $sql = @mysql_query("SELECT * FROM aday_yorum WHERE id='{$id}'");
  $assoc = @mysql_fetch_assoc($sql);
  return $assoc;
 }
 ...
 function aday_duzenle($id,$aday,$kategori,$oneren,$oneren_eposta,$oneri_metni){
  if(isset($id) AND isset($aday) AND isset($kategori) AND isset($oneren) AND isset($oneren_eposta) AND isset($oneri_metni)){
   $sql = @mysql_query("UPDATE `aday` SET `ad_soyad`='{$aday}', `kategori`='{$kategori}', `oneren_ad`='{$oneren}', `oneren_eposta`='{$oneren_eposta}', `durum`='0', `oneri_metni`='".html($oneri_metni)."' WHERE `id`='{$id}'");
  }
 ...
 function yorum_duzenle($id,$yorum_adi,$yorum_eposta,$yorum){
  if(isset($id) AND isset($yorum_adi) AND isset($yorum_eposta) AND isset($yorum)){
   $sql = @mysql_query("UPDATE `aday_yorum` SET `ekleyen_ad`='{$yorum_adi}', `ekleyen_eposta`='{$yorum_eposta}', `yorum`='".html($yorum)."', `aktif`='0' WHERE `id`='{$id}'");
  }

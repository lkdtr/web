<?php
 ...
 function goster_kategori($id = ""){
    $sql = @mysql_query("SELECT * FROM kategori");
 	...
 	$sqlb = @mysql_query("SELECT id,ad_soyad FROM 2006_aday WHERE kategori='{$id}' AND durum='1' ORDER by 'ad_soyad' ASC");
 ...
 function goster_aday_liste(){
  ...
  while($bgs = @mysql_fetch_assoc($sql)){
   $sqll = @mysql_query("SELECT * FROM 2006_aday WHERE kategori='{$bgs['id']}' AND durum='1' ORDER by 'ad_soyad' ASC");
   ?>
   <table align="center" border="0" cellpadding="4" cellspacing="1" width="96%">
      <tbody>
        <tr>
 ...
 function goster_aday($id = ""){
  $sql = @mysql_query("SELECT * FROM 2006_aday WHERE id='{$id}' AND durum='1'");
  $sqlw = @mysql_query("SELECT * FROM 2006_aday_yorum WHERE aday_id='{$id}' AND aktif='1'");
 ...
 function aday_ekle($aday,$kategori,$oneren,$oneren_eposta,$oneri_metni){
  if(isset($aday) AND isset($kategori) AND isset($oneren) AND isset($oneren_eposta) AND isset($oneri_metni)){
   $sql = @mysql_query("INSERT INTO `2006_aday` (`id`,`ad_soyad`,`kategori`,`oneren_ad`,`oneren_eposta`,`oneri_metni`,`durum`) VALUES ('', '{$aday}', '{$kategori}', '{$oneren}', '{$oneren_eposta}', '".html($oneri_metni)."', '0')");
  }
  if($sql){$ok = "pl";}
  elseif(!$sql){$ok = "pll";}
  else{$ok = "pll";}
 ...
 function penguen_adi($id){
  $sql = @mysql_query("SELECT id,ad_soyad FROM 2006_aday WHERE id='{$id}'");
 ...
 function yorum_ekle($penguen,$yorum_adi,$yorum_eposta,$yorum){
  if(isset($penguen) AND isset($yorum_adi) AND isset($yorum_eposta) AND isset($yorum)){
   $sql = @mysql_query("INSERT INTO `2006_aday_yorum` (`id`,`aday_id`,`ekleyen_ad`,`ekleyen_eposta`,`yorum`,`tarih`,`aktif`) VALUES ('', '{$penguen}', '{$yorum_adi}', '{$yorum_eposta}', '".html($yorum)."', '', '0')");
  }
  if($sql){$ok = "pl";}
  elseif(!$sql){$ok = "pll";}
  else{$ok = "pll";}
            ...
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                ...
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM 2006_aday_yorum WHERE aktif='0'");
            ...
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                ...
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM 2006_aday_yorum WHERE aktif='1' ORDER by 'id' ASC");
 ...
 function onayla_yorum($id,$islem){
  if($islem == 1){
   $sql = @mysql_query("UPDATE 2006_aday_yorum SET aktif='1' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=yorum"><?
  }
  elseif($islem == 0){
   $sql = @mysql_query("DELETE FROM 2006_aday_yorum WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=yorum"><?
  }
  elseif($islem == 2){
   $sql = @mysql_query("UPDATE 2006_aday_yorum SET aktif='0' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=yorum"><?
  }
  else{
            ...
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                ...
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM 2006_aday WHERE durum='0'");
            ...
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
              <tbody>
                ...
                <tr>
                  <td class="menu">
<?php
  $sqlw = @mysql_query("SELECT * FROM 2006_aday WHERE durum='1' ORDER by 'ad_soyad' ASC");
 ...
 function onayla_aday($id,$islem){
  if($islem == 1){
   $sql = @mysql_query("UPDATE 2006_aday SET durum='1' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=aday"><?
  }
  elseif($islem == 0){
   $sql = @mysql_query("DELETE FROM 2006_aday WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=aday"><?
  }
  elseif($islem == 2){
   $sql = @mysql_query("UPDATE 2006_aday SET durum='0' WHERE id='{$id}'");
   ?><meta http-equiv="refresh" content="0;URL=onayla.php?tip=aday"><?
  }
  else{
   ?><meta http-equiv="refresh" content="0;URL=index.php"><?
 ...
 function penguen_bilgi($id){
  $sql = @mysql_query("SELECT * FROM 2006_aday WHERE id='{$id}'");
  $assoc = @mysql_fetch_assoc($sql);
  return $assoc;
 }
 
 function yorum_bilgi($id){
  $sql = @mysql_query("SELECT * FROM 2006_aday_yorum WHERE id='{$id}'");
  $assoc = @mysql_fetch_assoc($sql);
  return $assoc;
 }
 ...
 function aday_duzenle($id,$aday,$kategori,$oneren,$oneren_eposta,$oneri_metni){
  if(isset($id) AND isset($aday) AND isset($kategori) AND isset($oneren) AND isset($oneren_eposta) AND isset($oneri_metni)){
   $sql = @mysql_query("UPDATE `2006_aday` SET `ad_soyad`='{$aday}', `kategori`='{$kategori}', `oneren_ad`='{$oneren}', `oneren_eposta`='{$oneren_eposta}', `durum`='0', `oneri_metni`='".html($oneri_metni)."' WHERE `id`='{$id}'");
  }
 ...
 function yorum_duzenle($id,$yorum_adi,$yorum_eposta,$yorum){
  if(isset($id) AND isset($yorum_adi) AND isset($yorum_eposta) AND isset($yorum)){
   $sql = @mysql_query("UPDATE `2006_aday_yorum` SET `ekleyen_ad`='{$yorum_adi}', `ekleyen_eposta`='{$yorum_eposta}', `yorum`='".html($yorum)."', `aktif`='0' WHERE `id`='{$id}'");
  }
